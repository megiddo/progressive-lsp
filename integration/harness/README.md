...
1. Publish a musl-static `progressive-lsp` (`xtask dist --libc musl`) that passes `check-static`. After LOG-4 the core ELF is rusqlite-linked; `run-it1.sh matrix` runs `xtask check-static` on that Linux ELF (not a Mach-O).
...
Images stay userspaces: no `nodejs`, `openjdk`, `python3`, or `php`. Network is off at run time; bind-mount the ELF. IT-1.1 asserts a `serve-*.sqlite` under `$PREFIX/log/` after handshake. IT-1.7 usage stays on stderr (LOG-4 did not move it).

## Darwin host

`run-it1.sh auto` builds the native Mach-O and runs **host_smoke** only. That is not IT-1.1. Docker + musl ELF are the distro gate — same class as the M0 musl gap. Do not run `check-static` on the Mach-O and call it green. The crate unit test `serve_handshake_writes_one_wal_sqlite_log_repository` covers the WAL path on tempfile.
