...
pub fn spawn_control_accept(listener: UnixListener, server: Arc<ControlServer>, host: Arc<ServeHost>) {
    thread::spawn(move || {
        ...
        loop {
            match listener.accept() {
                Ok((stream, _)) => {
                    ...
                    thread::spawn(move || {
                        let _ = handle_control_conn(stream, srv, plane);
                    });
                }
                Err(_) => break,
            }
    ...
    loop {
        match stream.read(&mut tmp) {
            ...
            Ok(n) => {
                ...
                drain_frames(&mut buf, &server, &host, &mut stream)?;
            }
            Err(e)
...
mod tests {
    ...
    fn bind_and_one_rpc_round_trip() {
        ...
        spawn_control_accept(listener, server, host);
