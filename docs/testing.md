...
`xtask musl` dest convention (HOST-2): `target/musl/<triple>/progressive-lsp` for `x86_64-unknown-linux-musl` and `aarch64-unknown-linux-musl`. `xtask pack` dest convention (HOST-3 slim / HOST-7 full): `target/musl/<triple>/engines/<pack>/<binary>` for slim packs (`python`/`ty`, `rust`/`rust-analyzer`, `phpantom`/`phpantom`, `biome`/`biome`, `superhtml`/`superhtml`) and full packs (`gopls`/`gopls`, `tsgo`/`tsgo`, `zls`/`zls`, `clangd`/`clangd`). clangd cache key is `target/pack-cache/clangd/<sha>/<triple>/clangd` (COPY hit; miss is documented, not cmake). `xtask runtime-image` dest convention (HOST-4): stage those dests into `target/runtime-image/<triple>/` (`prefix/` + `tmp/`) and `docker build -t progressive-lsp-runtime:local` (`CommandDockerPort` / `DockerPort::tag_image`). Full packs are optional copies. Image layout is `PrefixLayout` under `/opt/plsp`. Production extract is `docker build --output` (`CommandDockerPort` / `DockerPort::extract`). Crate tests cover [`MuslBuildPlan`](design-patterns.md) / [`PackBuildPlan`](design-patterns.md) / [`RuntimeImagePlan`](design-patterns.md) + `RecordingDockerPort` without a Docker daemon; that fixture ELF is **not** a musl green. Do not run `check-static` on a Darwin Mach-O or a Darwin `xtask dist` stub and call it green. After a real extract, run `xtask check-static` on those dest paths (orchestrator proof, not a cargo test). A tagged runtime image is HOST-4 proof, not a cargo test. Missing daemon / qemu/amd64 fail is a documented gap. HOST-3 live table: rust packs (ty, rust-analyzer, phpantom, biome) PASS both triples; `superhtml` PASS both triples (x86_64 via host-native `linux/arm64` + Zig `-Dtarget=x86_64-linux-musl`, not qemu). HOST-7 live table is in [milestones.md](milestones.md). The runtime Dockerfile must not compile; it only copies prebuilt ELFs. PR CI must not compile LLVM from scratch.
