...
`xtask musl` dest convention (HOST-2): `target/musl/<triple>/progressive-lsp` for `x86_64-unknown-linux-musl` and `aarch64-unknown-linux-musl`. `xtask pack` dest convention (HOST-3): `target/musl/<triple>/engines/<pack>/<binary>` for slim packs (`python`/`ty`, `rust`/`rust-analyzer`, `phpantom`/`phpantom`, `biome`/`biome`, `superhtml`/`superhtml`). `xtask runtime-image` dest convention (HOST-4): stage those dests into `target/runtime-image/<triple>/` (`prefix/` + `tmp/`) and `docker build -t progressive-lsp-runtime:local` (`CommandDockerPort` / `DockerPort::tag_image`). Image layout is `PrefixLayout` under `/opt/plsp`. Production extract is `docker build --output` (`CommandDockerPort` / `DockerPort::extract`). Crate tests cover [`MuslBuildPlan`](design-patterns.md) / [`PackBuildPlan`](design-patterns.md) / [`RuntimeImagePlan`](design-patterns.md) + `RecordingDockerPort` without a Docker daemon; that fixture ELF is **not** a musl green. Do not run `check-static` on a Darwin Mach-O or a Darwin `xtask dist` stub and call it green. After a real extract, run `xtask check-static` on those dest paths (orchestrator proof, not a cargo test). A tagged runtime image is HOST-4 proof, not a cargo test. Missing daemon / qemu/amd64 fail is a documented gap. HOST-3 live table: rust packs (ty, rust-analyzer, phpantom, biome) PASS both triples; `superhtml` PASS aarch64 and MISS x86_64 under Darwin qemu (Zig `access()` `Unexpected` on `.zig-cache`). Heavy packs (clangd/tsgo/gopls/zls) fail closed on HOST-3. The runtime Dockerfile must not compile; it only copies prebuilt ELFs.
