...
`./build lsp {arch} --flavor dogfood` (POST-BUILD): rebuilds slim + full pack dests, fail-closes before the runtime image if any dogfood dest is missing (clangd cites pack-cache / cache-fill). Default `--flavor slim` only rebuilds slim packs; image stamp uses slim pack dest bytes only.
