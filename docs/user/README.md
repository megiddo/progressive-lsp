...
- Weak go-to-definition usually means that language’s pack is not installed — install it, or keep using the simpler built-in index. The server does **not** fail the request: T2/T1 still answer. Sqlite `operation=resolve` **info** “pack skipped” is once per `(language, package)` — a second go-to-definition does not duplicate that row.
- Product logs are a SQLite WAL file under `$PREFIX/log/` named `serve-<unix_ms>-<pid>.sqlite` (plus `-wal` / `-shm` while the process is up). `$PREFIX` is the home from Install (`$HOME/.progressivelsp`, `PROGRESSIVE_LSP_HOME`, or `--prefix`). Override the file with `PROGRESSIVE_LSP_LOG` set to an **absolute** path. If the primary `serve-*.sqlite` is missing after `serve` ran, sqlite **open failed**: look next for `serve-fallback-*.sqlite` in the same `log/` directory, then the temp WAL `progressive-lsp-serve-<unix_ms>-<pid>.sqlite` (`progressive-lsp-serve-*.sqlite`) under the process temp directory. `serve` **stdout** stays JSON-RPC — do not look for logs there. `--help` / usage stay on **stderr**.
- Typed navigation missing after `serve` started: query that sqlite file for `operation=spawn` and `component=engine`. A **warn** row is a missing pack, hash mismatch, stub refuse (“reserved for Linux CI / Docker”), crash, or script abort — not a live engine child. When a pack later has a stderr pipe, a log file, or a proxied `window/logMessage` / `$/logTrace`, those lines are **third-party** `operation=spawn` (never engine **stdout**, which stays JSON-RPC). This host still does not exec stub packs. `operation=resolve` **info** “pack skipped” is the T3 miss (T2/T1 still work). `operation=initialize` **warn** is a script `on_bootstrap` abort or `InitializeFailed` (the editor also sees JSON-RPC `-32002`). `operation=initialize` **info** is a successful handshake. `operation=index` **info** is a script skip of a package or dropped workspace root. Stub packs never exec on this host.
- FilesSince catch-up incomplete: sqlite `operation=filesSince` **info** when the answer is truncated — call again with the returned generation, or rescan. Unknown `languageId` is `operation=resolve` **info** once per id. `shutdown` is **debug** completeness, not a failure.
- JSON-RPC parse, missing `Content-Length`, or mux framing problems: `operation=protocol` **warn** (method-not-found is **info**). Rows name the method or error class — never the JSON-RPC body.
- Control socket bind/accept failures, oversized Envelope, unknown method, or `SetConfig` / `InstallPacks` / `ReloadConfig` / `ReloadScripts` `Status::error`: `operation=control`. `--control-fd` is parsed then ignored (`pending`); that drop is one **warn**. Do not look for Envelope **bodies** in sqlite.
- `install` hash mismatch or verify refuse: `operation=install` **warn** with expected/actual **hex**, before the tmp/dest file is removed. Blob bytes are not logged.
