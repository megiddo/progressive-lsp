...
- Typed navigation missing after `serve` started: query that sqlite file for `operation=spawn` and `component=engine`. A **warn** row is a missing pack, hash mismatch, stub refuse (“reserved for Linux CI / Docker”), crash, or script abort — not a live engine child. `operation=initialize` **warn** is a script `on_bootstrap` abort. `operation=index` **info** is a script skip of a package or dropped workspace root. Stub packs never exec on this host.
