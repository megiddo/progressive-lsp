...
- Product logs are a SQLite WAL file under `$PREFIX/log/` named `serve-<unix_ms>-<pid>.sqlite` (plus `-wal` / `-shm` while the process is up). `$PREFIX` is the home from Install (`$HOME/.progressivelsp`, `PROGRESSIVE_LSP_HOME`, or `--prefix`). Override the file with `PROGRESSIVE_LSP_LOG` set to an **absolute** path. If that file is missing after `serve` ran, sqlite **open failed**: look for `serve-fallback-<unix_ms>-<pid>.sqlite` in the same `log/` directory, then `progressive-lsp-serve-<unix_ms>-<pid>.sqlite` under the process temp directory. `serve` **stdout** stays JSON-RPC — do not look for logs there. `--help` / usage stay on **stderr**.
- Typed navigation missing after `serve` started: query that sqlite file for `operation=spawn` and `component=engine`. A **warn** row is a missing pack, hash mismatch, stub refuse (“reserved for Linux CI / Docker”), crash, or script abort — not a live engine child. `operation=resolve` **info** “pack skipped” is the T3 miss (T2/T1 still work). `operation=initialize` **warn** is a script `on_bootstrap` abort or `InitializeFailed` (the editor also sees JSON-RPC `-32002`). `operation=initialize` **info** is a successful handshake. `operation=index` **info** is a script skip of a package or dropped workspace root. Stub packs never exec on this host.
