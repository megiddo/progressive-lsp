...
`--mux` is the container Strategy (`ServeMode::Mux` / `MuxStdio`). Native Darwin open keeps the Unix socket. `pending_mux` only when mux is advertised but not selected.
...
- Status strip: three **buttons** T1 / T2 / T3 (`processing` / `done` / `not supported` / `skipped` / `n/a`). Click opens `StatusModal` with that tier’s journal. Folder open starts workspace ingest in `initialize`; the strip paints T1/T2 from that ingest with no file focused. Opening a file is not required to start T2. After T2-COV, every v1 language has T2 (no `n/a` except `plaintext`). T3 stays `n/a` until a language is known. Native open on non-Linux skips T3 (`open folder in container`). C# T3 is `not supported`. Java T3 follows the static pack on the Linux host. Stub refuse is `skipped`, not `done`.
- File menu: On non-Linux, **Open Folder in Container…** is listed first (product default). **Open Folder…** is optional native T1/T2. Linux: **Open Folder…** is the full host only. Container open launches one Linux serve host and opens a **Container launch** modal with live preflight/launch steps. Tests use `FakeRuntime` (no Docker daemon). Container attach is `DockerRunPlan` → `MuxStdio::from_command` (`docker run -i --rm`, **bind-mount identity** so `file:` URIs match native open, `ServeMode::Mux`, `serve --prefix /opt/plsp --mux`). Native attach is unchanged (`ControlSocket`). Never two serves. **No URI rewriter** ([../t3-linux-hosts.md](../t3-linux-hosts.md)). Heavy packs are host7.
