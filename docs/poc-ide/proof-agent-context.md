...
3. **Implement** that milestone’s WPs only (spawn children if needed). Do not implement a later slice’s types (LSP IO threads, IndexStatus ingest fields, DiscoverOffer menus, highlight cache, tree-expand workers).
