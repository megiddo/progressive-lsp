...
`xtask dist` **slim** (default, also `--pack slim` / `--slim`) writes python, rust, phpantom, superhtml, biome, **java** stubs. **Full** (`--pack full` / `--full`) adds clangd, tsgo, gopls, zls. Slim is the Java-only default and excludes those heavy packs. csharp-ls is **not** a pack (T1/T2 ceiling). The Java T3 pack is a static musl ELF (Graal native-image, pack `java` / binary `javacs`); it belongs in **slim**. Darwin `xtask dist` still writes stubs + `DARWIN_CI_GAP.txt` only — do not confuse stub hashes with `check-static` greens. HOST-3 builds real slim musl packs (`phpantom`, `superhtml`, `biome` plus M3 ty/RA) via `xtask pack` / Docker extract to `target/musl/<triple>/engines/<pack>/<binary>`. JAVA-T3.2 aarch64 Java T3 is a **libc exception** (native-image, not fully static) until Graal fully-static ARM — [t3-linux-hosts.md](t3-linux-hosts.md). x86_64 stays fully static. HOST-7 builds full packs when present. Missing clangd cache is a documented miss — PR CI must not cmake LLVM. `--cache-fill clangd` is a dedicated job, not the default pack. Fail closed: do not ship dynamic backends. Project `go` / `zig` on PATH are project artifacts. Zig/Go/Graal toolchains may exist **inside the pack build container**.
