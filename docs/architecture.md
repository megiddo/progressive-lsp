...
`cargo xtask` only. musl default. `xtask musl` builds only `--bin progressive-lsp` in `docker/rust-musl.Dockerfile` and extracts to `target/musl/<triple>/progressive-lsp` (`docker build --output`), then `check-static`. `xtask pack` builds slim engine packs (ty, rust-analyzer, phpantom, biome, superhtml) via `docker/engine-pack.Dockerfile` / `docker/engine-pack-zig.Dockerfile`, extracts to `target/musl/<triple>/engines/<pack>/<binary>`, then `check-static`. Pins are git SHAs in `xtask/pack-pins.toml` (not core crate semver). Those ELFs are gitignored build artifacts — do not commit them. `xtask dist --libc musl|glibc-static --dest DIR` writes per-triple tarballs + SHA256 + `manifest.json`. Darwin `xtask dist` **pack** payloads are stubs (not musl ELFs; not `check-static` greens). Linux CI publishes the real `x86_64-unknown-linux-musl` / `aarch64-unknown-linux-musl` tarballs. Engine packs are **cached, content-addressed jobs** keyed by upstream git SHA in pack manifests (independent of core crate **0.1.0**). PR CI does not compile LLVM. Allocator: `xtask bench-alloc` → `xtask/allocator-matrix.toml`; `dist` only reads that file ([testing.md](testing.md)).
