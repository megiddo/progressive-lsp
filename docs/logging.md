...
**LOG-0–LOG-5 are signed off.** Do not reopen them. Remaining operational silent paths are LOG-6+ ([coverage matrix](#coverage-matrix-zero-blind-spots)). After LOG-11, a silent operational class in that matrix is a defect.
...
| `FilesSinceAnswer.truncated` | `FilesSinceJournal` / `ServeHost::files_since` | info | `filesSince` | first-party | LOG-8 | `FakeLog` |
