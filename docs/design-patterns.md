...
| `PackKind` | Value object | `rust`, `zig`, `go`, `graal` (`native-image` alias), `cached`, or `cmake`; unknown kind fail closed; host php/Node/JVM/CPython forbidden as our runtime; `graal` is Java T3 native-image (build-time JDK only; dest is a static musl ELF, never a JAR); `cached` is clangd default (cache COPY, never cmake); `cmake` is clangd `--cache-fill` only |
...
| `PackOutcome` | Value object / Domain Result | `Pass` after extract or cache COPY; `Miss` is a documented clangd cache gap **or** JAVA-T3.2 aarch64 Graal musl miss (not cmake, not a JAR/JVM, not a Mach-O green, not `Err`) |
...
| `PackImageCopy` | Value object | one pack ELF to copy (`pack`, `binary`, `src`, required); dest under `/opt/plsp/engines/<pack>/<binary>`; slim (ty, rust-analyzer, phpantom, biome, superhtml) required on both triples; `javacs` required on x86_64, optional omit on aarch64 (JAVA-T3.2 Graal musl static is x64 only); full (clangd/tsgo/gopls/zls) always optional |
