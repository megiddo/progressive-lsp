...
| `ExplicitPacks`, `CensusSelector` | Strategy (`PackSelector`) | Explicit CSV vs census; Java census selects the static Java T3 pack; C# census selects nothing (T1/T2 ceiling) |
...
| `ControlServer` | Facade | Proto RPCs; same domain services as LSP, different encoding; takes `Arc<dyn LogPort>` (LOG-7) or emit from `ServeHost` plane; **mux `--mux` and Unix `--control-socket` both `with_plane(ServeHost)`** — mux without a plane answers IndexStatus as ingest `not_started`; unknown method / `Status::error` without logging body bytes |
...
| `PackKind` | Value object | `rust`, `zig`, `go`, `graal` (`native-image` alias), `cached`, or `cmake`; unknown kind fail closed; host php/Node/JVM/CPython forbidden as our runtime; `graal` is Java T3 native-image (build-time JDK only; dest is a static musl ELF on x86_64, native-image that may need libc on aarch64, never a JAR); `cached` is clangd default (cache COPY, never cmake); `cmake` is clangd `--cache-fill` only |
...
| `PackBuildPlan` | Value object | pack name, binary name, triple, platform, dockerfile, dest `target/musl/<triple>/engines/<pack>/<binary>`, pinned SHA, cache key `sha:triple`; rust/go/graal/cached/cmake `--platform` follows the triple (`linux/amd64` / `linux/arm64`; qemu amd64 on Darwin is OK for those); **zig** (superhtml, zls) uses **host-native** `--platform` + `ZIG_TARGET` (Zig cross-compile; qemu `linux/amd64` is ENOSYS `faccessat` and is not used); `for_pin_on_host_arch` injects the host ISA so unit tests lock both `linux/amd64` (native CI, no qemu) and `linux/arm64` argv without a daemon; Darwin unit tests name the pattern and cover slim **and** heavy plans (gopls/tsgo/zls/clangd) **and both zig triples** **and the graal `java` pack** without docker; unknown packs fail closed; clangd uses content-addressed cache (`--cache-fill`); default `xtask pack` never cmake; `javacs` dest is required on both triples; Graal docker args are per-triple (`GRAAL_TAG` / `NATIVE_IMAGE_FLAGS`: x86_64 muslib + `--static --libc=musl`, aarch64 `25.0.0-ol9` + `-H:+StaticExecutableWithDynamicLibC`); aarch64 javacs may need libc (not a Miss, never a JAR / JVM) |
...
| `PackOutcome` | Value object / Domain Result | `Pass` after extract or cache COPY; `Miss` when clangd cache ELF absent until `--cache-fill` (not aarch64 java, not a JAR/JVM, not a Mach-O green, not `Err`) |
| `RuntimeImagePlan` | Value object | platform + triple + dockerfile (`runtime.Dockerfile` scratch on x86_64, `runtime-aarch64.Dockerfile` Rocky 9 minimal on aarch64 for javacs libc) + core dest + pack dests + image tag `progressive-lsp-runtime:local`; prefix `/opt/plsp`; unit tests cover both triples without docker; unknown triple / missing dockerfile / missing required core ELF fail closed; slim including `javacs` required both triples; POC dogfood image requires tsgo/gopls/zls/**clangd** |
| `PackImageCopy` | Value object | one pack ELF to copy (`pack`, `binary`, `src`, required); dest under `/opt/plsp/engines/<pack>/<binary>`; slim (ty, rust-analyzer, phpantom, biome, superhtml, `javacs`) required on both triples; aarch64 javacs libc exception is not a Miss; POC dogfood image requires tsgo/gopls/zls/**clangd** |
| `StaticCheckPolicy` | Value object / Strategy | `FullyStatic` is `xtask check-static` / `check_path` (no PT_INTERP, no `DT_NEEDED`); `NativeImageLibc` / `check_native_image_libc` is the aarch64 javacs exception (libc `DT_NEEDED` / PT_INTERP allowed; `libjvm`, JAR, Mach-O fail closed). x86_64 java and every other pack stay `FullyStatic` |
...
| `DiscoverOffer` | Value object | One LSP method + `min_tier` + language `ceiling`; C# ceiling is `graph`; Java ceiling is `types` (static pack); typed-only methods have `min_tier == types` |
...
| `MuxStdio` / `MuxLsp` / `MuxControl` | Adapter | One stdio pipe, protocol `MuxFrame` (`u8` + `u32be` + payload). Channel 0 = opaque JSON-RPC body (same as `serve_mux`, no Content-Length wrapper). Channel 1 = length-prefixed Envelope. A reader thread demuxes into per-channel inboxes so control `wait` cannot starve LSP `request` (no `thread::sleep`). Inboxes + fail + EOF share one mutex the condvar waits on (no lost wakeup). Peer EOF is `reader_done`, not `failed`, so writes still work after a Cursor is exhausted; unknown channel and payload > 16 MiB fail closed. Pair / Cursor tests; no daemon |
...
| `TierCell` / `TierCellKind` / `TierCellState` | Value objects | Cell is T1/T2/T3 × `processing` / `done` / `not supported` / `skipped` / `n/a`; `n/a` is waiting; C# T3 is `not supported`; after T2-COV no v1 language has matrix-no-T2; native non-Linux T3 is `skipped`; stub refuse is `skipped`, never `done` |
...
| `DiscoverMenu` / `DiscoverMenuItem` / `MenuDisableReason` | Value objects | Same ground truth for Navigate and context menus; ingest is `PackageTierMap::ingest_for_strip` (Ready + no IndexStatus is Done, matching the strip) so container mux cannot leave all items on `building T1 index`; disabled labels are `connecting language server` / `building T1 index` / `waiting for server` / `needs T2` / `needs T3` / `not supported` / `T3 skipped (stub pack)` / `open folder in container`; `to_io_request` is `None` while disabled so FakeLsp is not called |
...
| `LspFlavor` | Value object | `slim` (default) or `dogfood` for `./build lsp`; dogfood rebuilds [`full_pack_names`](design-patterns.md) and stales the runtime image stamp on full-pack dest bytes. |
| `LspFlags` | Value object | `./build lsp {arch} [--force] [--flavor slim\|dogfood]`; `--force` treats every artifact as stale; unknown extra args fail closed. |
| `LspArtifact` | Value object | One dest: musl core ELF, pack ELF, or runtime image stamp; identity is kind + triple (+ pack name; image includes flavor). |
| `LspArtifactStamp` | Value object | SHA-256 hex of that artifact’s inputs (core: Rust sources + `Cargo.lock` + musl dockerfile; pack: **that pack’s `[[pack]]` table** + the `[toolchain.*]` section that pack’s kind uses + that pack’s dockerfile — not the entire `pack-pins.toml`; image: runtime dockerfile + dest ELF bytes for packs in the selected [`LspFlavor`](design-patterns.md)). Equality is digest equality. Changing `[[pack]] java` must not stale phpantom. |
