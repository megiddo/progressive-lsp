...
| `PackBuildPlan` | Value object | pack name, binary name, triple, platform, dockerfile, dest `target/musl/<triple>/engines/<pack>/<binary>`, pinned SHA, cache key `sha:triple`; rust/go/cached/cmake `--platform` follows the triple (`linux/amd64` / `linux/arm64`); **zig** (superhtml, zls) uses **host-native** `--platform` + `ZIG_TARGET` (Zig cross-compile; qemu `linux/amd64` is ENOSYS `faccessat`); Darwin unit tests name the pattern and cover slim **and** heavy plans (gopls/tsgo/zls/clangd) **and both zig triples** without docker; unknown packs fail closed; clangd cache miss is a documented gap, not cmake |
