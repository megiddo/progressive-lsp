...
| `PackOutcome` | Value object / Domain Result | `Pass` after extract or cache COPY; `Miss` is a documented clangd cache gap only (not aarch64 java, not cmake, not a JAR/JVM, not a Mach-O green, not `Err`) |
| `RuntimeImagePlan` | Value object | platform + triple + dockerfile (`runtime.Dockerfile` scratch on x86_64, `runtime-aarch64.Dockerfile` Rocky 9 minimal on aarch64 for javacs libc) + core dest + pack dests + image tag `progressive-lsp-runtime:local`; prefix `/opt/plsp`; unit tests cover both triples without docker; unknown triple / missing dockerfile / missing required core ELF fail closed; slim including `javacs` required both triples; full packs optional (HOST-7 / clangd cache miss) until POC-REST promotes tsgo/gopls/zls |
| `PackImageCopy` | Value object | one pack ELF to copy (`pack`, `binary`, `src`, required); dest under `/opt/plsp/engines/<pack>/<binary>`; slim (ty, rust-analyzer, phpantom, biome, superhtml, `javacs`) required on both triples; aarch64 javacs libc exception is not a Miss; full (clangd/tsgo/gopls/zls) always optional |
...
| `LspFlags` | Value object | `./build lsp {arch} [--force]`; `--force` treats every artifact as stale; unknown extra args fail closed. |
| `LspArtifact` | Value object | One dest: musl core ELF, slim pack ELF, or runtime image stamp; identity is kind + triple (+ pack name). |
| `LspArtifactStamp` | Value object | SHA-256 hex of that artifact’s inputs (core: Rust sources + `Cargo.lock` + musl dockerfile; pack: **that pack’s `[[pack]]` table** + the `[toolchain.*]` section that pack’s kind uses + that pack’s dockerfile — not the entire `pack-pins.toml`; image: runtime dockerfile + dest ELF bytes). Equality is digest equality. Changing `[[pack]] java` must not stale phpantom. |
