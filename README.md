...
Internal design (implementers): [docs/README.md](docs/README.md). Stacked branches: v1 was `docs-0` → `m0`–`m6` (merged). Post-dev: `pd0` → `pd1`–`pd4`. POC IDE consumer: `ide0` → `ide5` — [docs/poc-ide/README.md](docs/poc-ide/README.md). Supported proof launch is `cargo xtask poc` (not bare `cargo run -p poc-ide`). See [docs/branching.md](docs/branching.md).
